...
if(!$permsobj->hasPerm($user->user_id, "perm_logs") AND $user->user_rank != 0) { echo "<div class='content_box'>You do not have Permission!</div>"; } else {	
	...
	echo '<div class="content_box">';
	echo '<span style="font-size: 22px;"><b>Cronjob Status</b></span><br /><hr><br /><div style="text-align:left;">';
	$res	=	$mysql->select("SELECT * FROM "._TABLE_LOG_."  ORDER BY id DESC LIMIT 10", true); 	
	if(is_array($res)) { 
		foreach ($res AS $key => $value) { 	
